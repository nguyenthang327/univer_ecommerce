...
<div class="table-responsive">
    <table class="table table-hover table-striped table-bordered table-valign-middle table-custom min-width-800">
        ...
        <tr>
            ...
            <th width="8%">{{ trans('language.thumbnail') }}</th>
            <th width="11%">{{ trans('language.category_name') }}</th>
            ...
            @foreach($data as $idx => $category)
                <tr class="{{ $category->parent_id ? 'bg-light' : 'bg-white' }}">
                    ...
                    <td class="text-center">
                        <a href="{{ isset($category->thumbnail) ? asset('storage/'. $category->thumbnail) : asset('images/no-image.png')  }}" class="fancybox2" data-fancybox-group="avatar-list-user" title="ID: {{$category->id}} - {{ $category->name }}">
                    ...
                    <td class="text-center text-nowrap">
                        @if($category->deleted_at == null)
                            <a href="{{ route('admin.user.edit',['id'=>$category->id]) }}" data-toggle='tooltip' title="{{trans('language.edit')}}" class="text-md text-primary mr-2"><i class="far fa-pen-alt"></i></a>
